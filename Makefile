...
lab-start: build ## Start a lab (PROJECT=path PROFILE=name [BRANCH=name] [NAME=name] [FEATURE=lang] [BASE_PROFILE=name] [FIREWALL=1])
	...
	@$(BIN) start --project $(PROJECT) --profile $(PROFILE) \
		$(if $(BRANCH),--branch $(BRANCH)) $(if $(NAME),--name "$(NAME)") \
		$(if $(FEATURE),--feature $(FEATURE)) $(if $(BASE_PROFILE),--base-profile $(BASE_PROFILE)) \
